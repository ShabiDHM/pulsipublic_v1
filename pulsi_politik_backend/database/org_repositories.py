...
print("<<<<< EXECUTING REPOSITORIES.PY (Version: LATEST_FULL_WITH_DEBUG_AND_POPULATION_REFINEMENTS) >>>>>")
...
# LINE 5 (the problematic import) HAS BEEN REMOVED.
# Imports like MinistryRepository, get_previous_period_code are not needed here
# as they are defined within this file itself.
...
try:
    from ..config import Config # This import is fine and necessary
...
class BaseRepository:
    # ... (rest of BaseRepository class - keep as it was)
...
class MinistryRepository(BaseRepository):
    # ... (all your methods: get_ministry_by_id, get_all_ministries_overview, etc., including the new get_all_pillar_scores_for_period)
    # ... (Ensure the content of these methods is exactly as per the last good version I sent for implementing "Outcome" scores) ...
    def get_ministry_by_id(self, ministry_id: int) -> Optional[Dict[str, Any]]: # ... (full method from before) ...
        try:
    ...
    def get_all_ministries_overview(self) -> List[Dict[str, Any]]: # ... (full method from before) ...
        ministries_list = []
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                query = """ SELECT ministry_id, name_en, name_sq, name_sr, abbreviation_en, abbreviation_sq, abbreviation_sr,
                            ...
                            FROM ministries ORDER BY name_en ASC """
    ...
    def get_indicators_for_ministry(self, ministry_id: int, assessment_date: Optional[str] = None) -> List[Dict[str, Any]]: # ... (full method from before) ...
        results_list = []
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
    ...
    def get_kpis_for_ministry(self, ministry_id: int, kpi_name_keys_list: Optional[List[str]] = None, period_code_filter: Optional[str] = None) -> List[Dict[str, Any]]: # ... (full method from before) ...
        results_list = []
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
    ...
    def get_recent_activities_for_ministry(self, ministry_id: int, limit: int = 5) -> List[Dict[str, Any]]: # ... (full method from before) ...
        results_list = []
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
    ...
    def get_dashboard_indicators_by_pillar(self, pillar_key_filter: str, period_code: Optional[str] = None) -> List[Dict[str, Any]]: # ... (full method from before) ...
        results_list = []
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                ...
                query = f"""
                    WITH MaxAssessmentDates AS (
                        ...
                        FROM ministry_indicators WHERE pillar_key = ? {cte_period_sql_condition} 
                    ...
                    JOIN MaxAssessmentDates mad ON mi.ministry_id = mad.ministry_id 
                                               ...
                                               {join_mad_period_sql_condition} 
                    WHERE mi.pillar_key = ? {where_mi_period_sql_condition}
    ...
    def get_all_pillar_scores_for_period(self, period_code: str) -> List[Dict[str, Any]]: # ... (full method from before) ...
        results_list = []
    ...
    def get_average_score_for_pillar_period(self, pillar_key: str, period_code: Optional[str]) -> Optional[float]: # ... (full method from before) ...
        if not period_code: return None
    ...
    def get_kpi_summary_for_dashboard(self, current_period_code: Optional[str]) -> Dict[str, Dict[str, Optional[float]]]: # ... (full method from before) ...
        summary = { "avgTransparency": {"value": None, "change": None}, "participationScore": {"value": None, "change": None}, "efficiencyRating": {"value": None, "change": None}, "overallOutcome": {"value": None, "change": None} }
        if not current_period_code: return summary
        kpi_to_pillar_map = { "avgTransparency": "Transparency", "participationScore": "Participation", "efficiencyRating": "Efficiency" }
        ...
        for kpi_key, pillar_key in kpi_to_pillar_map.items():
            ...
            if previous_period_code:
                ...
                if current_value is not None and previous_value is not None: summary[kpi_key]["change"] = round(current_value - previous_value, 1)
        current_vals_for_overall = [summary[k]["value"] for k in kpi_to_pillar_map if summary[k]["value"] is not None]
        if current_vals_for_overall: summary["overallOutcome"]["value"] = round(sum(current_vals_for_overall) / len(current_vals_for_overall), 1)
        if previous_period_code and summary["overallOutcome"]["value"] is not None:
            ...
            if valid_prev_overall_components:
                ...
                if summary["overallOutcome"]["value"] is not None and prev_overall_value is not None: summary["overallOutcome"]["change"] = round(summary["overallOutcome"]["value"] - prev_overall_value, 1)
        return summary
...
def _get_direct_db_connection_for_setup(db_path_for_setup: str):
    ...
    conn = sqlite3.connect(db_path_for_setup); conn.row_factory = sqlite3.Row; conn.execute("PRAGMA foreign_keys = ON;"); return conn

def init_db(db_path: Optional[str] = None):
    actual_db_path = db_path if db_path is not None else CONFIGURED_DB_PATH; print(f"Initializing database schema in '{actual_db_path}'..."); conn = _get_direct_db_connection_for_setup(actual_db_path); cursor = conn.cursor(); cursor.execute("DROP TABLE IF EXISTS ministry_activities"); cursor.execute("DROP TABLE IF EXISTS ministry_kpis"); cursor.execute("DROP TABLE IF EXISTS ministry_indicators"); cursor.execute("DROP TABLE IF EXISTS ministries"); print("Relevant tables dropped (if they existed)."); cursor.execute(''' CREATE TABLE ministries ( ministry_id INTEGER PRIMARY KEY AUTOINCREMENT, name_en TEXT NOT NULL UNIQUE, name_sq TEXT NOT NULL UNIQUE, name_sr TEXT UNIQUE, abbreviation_en TEXT, abbreviation_sq TEXT, abbreviation_sr TEXT, minister_name_en TEXT, minister_name_sq TEXT, minister_name_sr TEXT, category_key TEXT, cabinet_member_1_name_en TEXT, cabinet_member_1_name_sq TEXT, cabinet_member_1_name_sr TEXT, cabinet_member_2_name_en TEXT, cabinet_member_2_name_sq TEXT, cabinet_member_2_name_sr TEXT, cabinet_member_3_name_en TEXT, cabinet_member_3_name_sq TEXT, cabinet_member_3_name_sr TEXT, cabinet_member_4_name_en TEXT, cabinet_member_4_name_sq TEXT, cabinet_member_4_name_sr TEXT, cabinet_member_5_name_en TEXT, cabinet_member_5_name_sq TEXT, cabinet_member_5_name_sr TEXT, established_date TEXT, website_url TEXT, contact_email TEXT, contact_phone TEXT, last_profile_update TIMESTAMP DEFAULT CURRENT_TIMESTAMP ) '''); print("'ministries' table created."); cursor.execute(''' CREATE TABLE ministry_indicators ( indicator_log_id INTEGER PRIMARY KEY AUTOINCREMENT, ministry_id INTEGER NOT NULL, pillar_key TEXT NOT NULL, indicator_name_en TEXT, indicator_name_sq TEXT, indicator_name_sr TEXT, value REAL NOT NULL, unit TEXT DEFAULT 'Score (0-100)', assessment_date TEXT NOT NULL, period_code TEXT, description_en TEXT, description_sq TEXT, description_sr TEXT, data_source_en TEXT, data_source_sq TEXT, data_source_sr TEXT, FOREIGN KEY (ministry_id) REFERENCES ministries (ministry_id) ON DELETE CASCADE ) '''); print("'ministry_indicators' table created."); cursor.execute(''' CREATE TABLE ministry_kpis ( kpi_id INTEGER PRIMARY KEY AUTOINCREMENT, ministry_id INTEGER NOT NULL, kpi_name_key TEXT NOT NULL, kpi_value_numeric REAL, kpi_value_text TEXT, unit TEXT, period_start_date TEXT NOT NULL, period_end_date TEXT NOT NULL, last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (ministry_id) REFERENCES ministries (ministry_id) ON DELETE CASCADE, UNIQUE (ministry_id, kpi_name_key, period_end_date) ) '''); print("'ministry_kpis' table created."); cursor.execute(''' CREATE TABLE ministry_activities ( activity_id INTEGER PRIMARY KEY AUTOINCREMENT, ministry_id INTEGER NOT NULL, activity_date TEXT NOT NULL, title_en TEXT NOT NULL, title_sq TEXT NOT NULL, title_sr TEXT, description_en TEXT, description_sq TEXT, description_sr TEXT, category_en TEXT, category_sq TEXT, category_sr TEXT, source_url TEXT, is_highlight INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (ministry_id) REFERENCES ministries (ministry_id) ON DELETE CASCADE ) '''); print("'ministry_activities' table created."); conn.commit(); conn.close(); print(f"Database '{actual_db_path}' initialized with new schema.")
...
def populate_all_ministry_data(db_path: Optional[str] = None):
    ...
    cursor = conn.cursor()
    ministries_data_source = [ {"names": {"en": "Ministry of Finance, Labour and Transfers", "sq": "Ministria e Financave, Punës dhe Transfereve", "sr": "Ministarstvo finansija, rada i transfera"}, "abbrs": {"en": "MFLT", "sq": "MFPT", "sr": "MFRT"}, "minister": {"en": "Hekuran Murati", "sq": "Hekuran Murati", "sr": "Hekuran Murati"}, "category_key": "economic", "cabinets": { "en": ["Hekuran Murati", "Agon Dobruna"], "sq": ["Hekuran Murati", "Agon Dobruna"], "sr": ["Hekuran Murati", "Agon Dobruna"] } }, {"names": {"en": "Ministry of Foreign Affairs and Diaspora", "sq": "Ministria e Punëve të Jashtme dhe Diasporës", "sr": "Ministarstvo spoljnih poslova i dijaspore"}, "abbrs": {"en": "MFA", "sq": "MPJD", "sr": "MSP"}, "minister": {"en": "Donika Gërvalla-Schwarz", "sq": "Donika Gërvalla-Schwarz", "sr": "Donika Gervala-Švarc"}, "category_key": "governance", "cabinets": { "en": ["Donika Gërvalla-Schwarz", "Kreshnik Ahmeti", "Liza Gashi"], "sq": ["Donika Gërvalla-Schwarz", "Kreshnik Ahmeti", "Liza Gashi"], "sr": ["Donika Gervala-Švarc", "Krešnik Ahmeti", "Liza Gaši"] } }, {"names": {"en": "Ministry of Justice", "sq": "Ministria e Drejtësisë", "sr": "Ministarstvo pravde"}, "abbrs": {"en": "MoJ", "sq": "MD", "sr": "MP"}, "minister": {"en": "Albulena Haxhiu", "sq": "Albulena Haxhiu", "sr": "Aljbulena Hadžiju"}, "category_key": "governance", "cabinets": { "en": ["Albulena Haxhiu", "Blerim Sallahu", "Vigan Qorrolli"], "sq": ["Albulena Haxhiu", "Blerim Sallahu", "Vigan Qorrolli"], "sr": ["Aljbulena Hadžiju", "Blerim Salahu", "Vigan Ćoroli"] } }, {"names": {"en": "Ministry of Defence", "sq": "Ministria e Mbrojtjes", "sr": "Ministarstvo odbrane"}, "abbrs": {"en": "MoD", "sq": "MM", "sr": "MO"}, "minister": {"en": "Ejup Maqedonci", "sq": "Ejup Maqedonci", "sr": "Ejup Maćedonci"}, "category_key": "governance", "cabinets": { "en": ["Ejup Maqedonci", "Shemsi Syla"], "sq": ["Ejup Maqedonci", "Shemsi Syla"], "sr": ["Ejup Maćedonci", "Šemsi Sila"] } }, {"names": {"en": "Ministry of Internal Affairs", "sq": "Ministria e Punëve të Brendshme", "sr": "Ministarstvo unutrašnjih poslova"}, "abbrs": {"en": "MIA", "sq": "MPB", "sr": "MUP"}, "minister": {"en": "Xhelal Sveçla", "sq": "Xhelal Sveçla", "sr": "Dželjalj Svečlja"}, "category_key": "governance", "cabinets": { "en": ["Xhelal Sveçla", "Bardhyl Dobra", "Blerim Gashani"], "sq": ["Xhelal Sveçla", "Bardhyl Dobra", "Blerim Gashani"], "sr": ["Dželjalj Svečlja", "Bardilj Dobra", "Blerim Gašani"] } }, {"names": {"en": "Ministry of Health", "sq": "Ministria e Shëndetësisë", "sr": "Ministarstvo zdravlja"}, "abbrs": {"en": "MoH", "sq": "MSH", "sr": "MZ"}, "minister": {"en": "Arben Vitia", "sq": "Arben Vitia", "sr": "Arben Vitia"}, "category_key": "social", "cabinets": { "en": ["Arben Vitia", "Dafina Gexha-Bunjaku", "Arsim Berisha"], "sq": ["Arben Vitia", "Dafina Gexha-Bunjaku", "Arsim Berisha"], "sr": ["Arben Vitia", "Dafina Gedža-Bunjaku", "Arsim Beriša"] } }, {"names": {"en": "Ministry of Education, Science, Technology and Innovation", "sq": "Ministria e Arsimit, Shkencës, Teknologjisë dhe Inovacionit", "sr": "Ministarstvo obrazovanja, nauke, tehnologije i inovacija"}, "abbrs": {"en": "MESTI", "sq": "MASHTI", "sr": "MONTI"}, "minister": {"en": "Arbërie Nagavci", "sq": "Arbërie Nagavci", "sr": "Arberie Nagavci"}, "category_key": "social", "cabinets": { "en": ["Arbërie Nagavci", "Dukagjin Pupovci", "Edona Maloku-Bërdyna"], "sq": ["Arbërie Nagavci", "Dukagjin Pupovci", "Edona Maloku-Bërdyna"], "sr": ["Arberie Nagavci", "Dukađin Pupovci", "Edona Maloku-Berdina"] } }, {"names": {"en": "Ministry of Culture, Youth and Sports", "sq": "Ministria e Kulturës, Rinisë dhe Sportit", "sr": "Ministarstvo kulture, omladine i sporta"}, "abbrs": {"en": "MCYS", "sq": "MKRS", "sr": "MKOS"}, "minister": {"en": "Hajrulla Çeku", "sq": "Hajrulla Çeku", "sr": "Hajrula Čeku"}, "category_key": "social", "cabinets": { "en": ["Hajrulla Çeku", "Daulina Osmani", "Sylejman Elshani"], "sq": ["Hajrulla Çeku", "Daulina Osmani", "Sylejman Elshani"], "sr": ["Hajrula Čeku", "Daulina Osmani", "Sulejman Elšani"] } }, {"names": {"en": "Ministry of Local Government Administration", "sq": "Ministria e Administrimit të Pushtetit Lokal", "sr": "Ministarstvo administracije lokalne samouprave"}, "abbrs": {"en": "MLGA", "sq": "MAPL", "sr": "MALS"}, "minister": {"en": "Elbert Krasniqi", "sq": "Elbert Krasniqi", "sr": "Eljbert Krasnići"}, "category_key": "governance", "cabinets": { "en": ["Elbert Krasniqi", "Arbër Vokrri"], "sq": ["Elbert Krasniqi", "Arbër Vokrri"], "sr": ["Eljbert Krasnići", "Arber Vokri"] } }, {"names": {"en": "Ministry of Environment, Spatial Planning and Infrastructure", "sq": "Ministria e Mjedisit, Planifikimit Hapësinor dhe Infrastrukturës", "sr": "Ministarstvo životne sredine, prostornog planiranja i infrastrukture"}, "abbrs": {"en": "MESPI", "sq": "MMPHI", "sr": "MZSPI"}, "minister": {"en": "Liburn Aliu", "sq": "Liburn Aliu", "sr": "Ljiburn Aljiu"}, "category_key": "infrastructure", "cabinets": { "en": ["Liburn Aliu", "Hysen Durmishi", "Avni Zogiani"], "sq": ["Liburn Aliu", "Hysen Durmishi", "Avni Zogiani"], "sr": ["Ljiburn Aljiu", "Hisen Durmiši", "Avni Zogjani"] } }, {"names": {"en": "Ministry of Agriculture, Forestry and Rural Development", "sq": "Ministria e Bujqësisë, Pylltarisë dhe Zhvillimit Rural", "sr": "Ministarstvo poljoprivrede, šumarstva i ruralnog razvoja"}, "abbrs": {"en": "MAFRD", "sq": "MBPZHR", "sr": "MPSRR"}, "minister": {"en": "Faton Peci", "sq": "Faton Peci", "sr": "Faton Peci"}, "category_key": "economic", "cabinets": { "en": ["Faton Peci", "Adonika K Hoxha", "Gazmend Gjushinca"], "sq": ["Faton Peci", "Adonika K Hoxha", "Gazmend Gjushinca"], "sr": ["Faton Peci", "Adonika K Hodža", "Gazmend Đušinca"] } }, {"names": {"en": "Ministry of Industry, Entrepreneurship and Trade", "sq": "Ministria e Industrisë, Ndërmarrësisë dhe Tregtisë", "sr": "Ministarstvo industrije, preduzetništva i trgovine"}, "abbrs": {"en": "MIET", "sq": "MINT", "sr": "MIPT"}, "minister": {"en": "Rozeta Hajdari", "sq": "Rozeta Hajdari", "sr": "Rozeta Hajdari"}, "category_key": "economic", "cabinets": { "en": ["Rozeta Hajdari", "Mentor Arifaj"], "sq": ["Rozeta Hajdari", "Mentor Arifaj"], "sr": ["Rozeta Hajdari", "Mentor Arifaj"] } }, {"names": {"en": "Ministry of Economy", "sq": "Ministria e Ekonomisë", "sr": "Ministarstvo ekonomije"}, "abbrs": {"en": "ME", "sq": "ME", "sr": "MEk"}, "minister": {"en": "Artane Rizvanolli", "sq": "Artane Rizvanolli", "sr": "Artane Rizvanoli"}, "category_key": "economic", "cabinets": { "en": ["Artane Rizvanolli", "Mentor Geci", "Getoar Mjeku"], "sq": ["Artane Rizvanolli", "Mentor Geci", "Getoar Mjeku"], "sr": ["Artane Rizvanoli", "Mentor Geci", "Getoar Mjeku"] } }, {"names": {"en": "Ministry for Communities and Returns", "sq": "Ministria për Komunitete dhe Kthim", "sr": "Ministarstvo za zajednice i povratak"}, "abbrs": {"en": "MCR", "sq": "MKK", "sr": "MZP"}, "minister": {"en": "Nenad Rašić", "sq": "Nenad Rašić", "sr": "Nenad Rašić"}, "category_key": "social", "cabinets": { "en": ["Nenad Rašić", "Radoica Radomirović", "Gazmen Salijević"], "sq": ["Nenad Rašić", "Radoica Radomirović", "Gazmen Salijević"], "sr": ["Nenad Rašić", "Radoica Radomirović", "Gazmen Salijević"] } }, {"names": {"en": "Ministry of Regional Development", "sq": "Ministria e Zhvillimit Rajonal", "sr": "Ministarstvo regionalnog razvoja"}, "abbrs": {"en": "MRD", "sq": "MZHR", "sr": "MRR"}, "minister": {"en": "Fikrim Damka", "sq": "Fikrim Damka", "sr": "Fikrim Damka"}, "category_key": "infrastructure", "cabinets": { "en": ["Fikrim Damka", "Ali Tafarshiku", "Agonislami Ferati"], "sq": ["Fikrim Damka", "Ali Tafarshiku", "Agonislami Ferati"], "sr": ["Fikrim Damka", "Ali Tafaršiku", "Agonislami Ferati"] } }
    ]
    ministry_ids_map = {}
    print(f"\nPopulating 'ministries' table with {len(ministries_data_source)} entries...")
    for item in ministries_data_source:
        padded_cabinets = {}
        ...
        try:
            cursor.execute(f""" INSERT INTO ministries (name_en, name_sq, name_sr, abbreviation_en, abbreviation_sq, abbreviation_sr, 
                                                        minister_name_en, minister_name_sq, minister_name_sr, category_key,
                                                        cabinet_member_1_name_en, cabinet_member_1_name_sq, cabinet_member_1_name_sr,
                                                        cabinet_member_2_name_en, cabinet_member_2_name_sq, cabinet_member_2_name_sr,
                                                        cabinet_member_3_name_en, cabinet_member_3_name_sq, cabinet_member_3_name_sr,
            ...
            ministry_ids_map[item["names"]["en"]] = cursor.lastrowid
    ...
    periods_for_data = { "q1-2023": (datetime.date(2023, 1, 1), datetime.date(2023, 3, 31)), "q2-2023": (datetime.date(2023, 4, 1), datetime.date(2023, 6, 30)), "q3-2023": (datetime.date(2023, 7, 1), datetime.date(2023, 9, 30)), "q4-2022": (datetime.date(2022, 10, 1), datetime.date(2022, 12, 31)) }
    ...
    for period_code, (start_date, end_date) in periods_for_data.items():
        ...
        for ministry_id in ministry_ids_map.values():
            for pillar_key in pillars_for_indicators:
                value = round(random.uniform(40.0, 90.0), 1)
                cursor.execute(""" INSERT INTO ministry_indicators ( ministry_id, pillar_key, indicator_name_en, indicator_name_sq, indicator_name_sr, value, unit, assessment_date, period_code, description_en, description_sq, description_sr, data_source_en, data_source_sq, data_source_sr) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) """, (ministry_id, pillar_key, f"{pillar_key} Index", f"Indeksi i {pillar_key}", f"{pillar_key} Indeks", value, 'Score (0-100)', assessment_date_for_period, period_code, f"Overall {pillar_key.lower()} performance index.", f"Indeksi i përgjithshëm i performancës {pillar_key.lower()}.", f"Sveukupni indeks učinka {pillar_key.lower()}.", "Simulated Data Source", "Burim i Simuluar", "Simulirani Izvor")); indicators_inserted_count += 1
    print(f"Ministry indicators (pillar scores) inserted: {indicators_inserted_count}.")

    kpi_definitions_for_details_page = [ 'website_transparency_score', 'info_requests_received_count', 'info_requests_processed_count', 'avg_response_time_days', 'document_accessibility_score', 'request_responsiveness_score', 'information_completeness_score', 'public_engagement_score', 'budget_utilization_percentage' ]
    ...
    for period_code_str, (period_s_obj, period_e_obj) in periods_for_data.items():
        period_start_str = period_s_obj.strftime('%Y-%m-%d'); period_end_str = period_e_obj.strftime('%Y-%m-%d')
        for ministry_id in ministry_ids_map.values():
            for kpi_name_key in kpi_definitions_for_details_page:
                kpi_val_num = None; kpi_unit = '%'; 
                if 'count' in kpi_name_key: kpi_val_num = random.randint(10, 200); kpi_unit = 'count'
                elif 'days' in kpi_name_key: kpi_val_num = round(random.uniform(1.0, 25.0), 1); kpi_unit = 'days'
                elif 'score' in kpi_name_key or 'percentage' in kpi_name_key: kpi_val_num = random.randint(30, 98)
                else: kpi_val_num = random.randint(0,100)
                try: cursor.execute("""INSERT INTO ministry_kpis (ministry_id, kpi_name_key, kpi_value_numeric, unit, period_start_date, period_end_date) VALUES (?,?,?,?,?,?)""", (ministry_id, kpi_name_key, kpi_val_num, kpi_unit, period_start_str, period_end_str)); kpis_inserted_count+=1
                except sqlite3.IntegrityError: pass
                except sqlite3.Error as e: print(f"  DB Error inserting KPI '{kpi_name_key}': {e}")
    ...
    for ministry_id in ministry_ids_map.values():
        for i in range(random.randint(2,5)): 
            activity_date_dt = datetime.date.today() - datetime.timedelta(days=random.randint(1, 270)); activity_date_str = activity_date_dt.strftime('%Y-%m-%d'); cat_en = random.choice(["Policy Update","Public Event","Report Release", "New Initiative", "Consultation Document"]); title_en = f"{cat_en}: Ministry {ministry_id} Focus - Ref {random.randint(100,999)}"; desc_en = f"Details for {title_en}."; cursor.execute("""INSERT INTO ministry_activities (ministry_id, activity_date, title_en, title_sq, title_sr, description_en, description_sq, description_sr, category_en, category_sq, category_sr, source_url) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)""", (ministry_id, activity_date_str, title_en, title_en.replace("(EN)","(SQ)").replace("Ministry","Ministria").replace("Focus Area","Fokus").replace("Ref","Ref"), title_en.replace("(EN)","(SR)").replace("Ministry","Ministarstvo").replace("Focus Area","Fokus Oblast").replace("Ref","Ref"), desc_en, desc_en.replace("(EN)","(SQ)").replace("ongoing efforts","përpjekjet e vazhdueshme").replace("public involvement","përfshirjen publike"), desc_en.replace("(EN)","(SR)").replace("ongoing efforts","tekuće napore").replace("public involvement","učešće javnosti"), cat_en, cat_en, cat_en, f"http://example.gov/activity/{ministry_id}/{random.randint(1000,9999)}")); activities_inserted_count+=1
...
if __name__ == '__main__':
    ...
    if not any(v['value'] for v in kpi_summary_test.values() if v and 'value' in v and v['value'] is not None): print(f"WARNING: KPI Summary for {test_period} seems to have all null values.")
    print(f"\nDashboard Indicators for Transparency, {test_period} (first 5 if any):")
    dash_indicators = repo.get_dashboard_indicators_by_pillar("Transparency", test_period)
    for i, indicator in enumerate(dash_indicators[:5]): print(f"  Row {i+1}: {indicator}")
    if not dash_indicators: print(f"WARNING: No dashboard indicators found for Transparency, {test_period}.")
    else: print(f"Total indicators found for Transparency, {test_period}: {len(dash_indicators)}")
    print(f"\nALL PILLAR SCORES for {test_period} (first 10 rows):")
    all_scores = repo.get_all_pillar_scores_for_period(test_period)
    for i, score_row in enumerate(all_scores[:10]): print(f"  Row {i+1}: {score_row}")
    if not all_scores: print(f"WARNING: No scores returned by get_all_pillar_scores_for_period for {test_period}.")
    else: print(f"Total rows from get_all_pillar_scores_for_period: {len(all_scores)}")
    test_ministry_id = 1
    print(f"\nKPIs for Ministry {test_ministry_id}, {test_period}:")
    kpis_for_ministry_test = repo.get_kpis_for_ministry(test_ministry_id, period_code_filter=test_period)
    if kpis_for_ministry_test: print(kpis_for_ministry_test[:3]) 
    else: print(f"WARNING: No KPIs found for Ministry {test_ministry_id}, {test_period}.")
